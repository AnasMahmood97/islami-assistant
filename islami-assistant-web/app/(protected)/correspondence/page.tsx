  ...
  return (
    <section dir="rtl" className="chat-pane text-right">
      ...
      <div className="space-y-3">
        {filtered.map((row) => (
          <article key={row.id} className="group rounded-2xl border border-[#E60000]/15 bg-white/90 p-3 transition-all duration-300 ease-out hover:shadow-[0_10px_24px_rgba(230,0,0,0.14)]">
            ...
            <button className="mt-2 text-sm text-[#9e1b1f] underline" onClick={() => setExpandedId((v) => (v === row.id ? null : row.id))}>
              {expandedId === row.id ? "إخفاء" : "المزيد"}
            </button>
            ...
            <div className="mt-2 flex gap-2">
              ...
              {isAdmin ? (
                <button
                  type="button"
                  className="text-sm text-[#E60000] admin-hover-action"
                  onClick={() => {
                    setEditingId(row.id);
                    setForm({
                      title: row.title,
